...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", _("Send RPC for non-default wallet on RPC server (argument is wallet filename in HSBCd directory, required if bitcoind/-Qt runs with multiple wallets)"));
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  HSBC-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  HSBC-cli [options] -named <command> [name=value] ... " + strprintf(_("Send command to %s (with named arguments)"), _(PACKAGE_NAME)) + "\n" +
                  "  HSBC-cli [options] help                " + _("List commands") + "\n" +
                  "  HSBC-cli [options] help <command>      " + _("Get help for a command") + "\n";
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to HSBC-cli command line.";
                        }
                    }
                } else {
                    // Result
