...
class SegWitTest(BitcoinTestFramework):
    ...
    def test_unnecessary_witness_before_segwit_activation(self):
        ...
        # Get the actual transaction from the accepted block to ensure we have the correct txid
        accepted_block_hash = self.nodes[0].getbestblockhash()
        accepted_block = self.test_node.request_block(int(accepted_block_hash, 16), MSG_BLOCK)
        # The transaction should be at index 1 (after coinbase)
        assert len(accepted_block.vtx) > 1, "Block should contain our transaction"
        accepted_tx = accepted_block.vtx[1]
        self.utxo.pop(0)
        self.utxo.append(UTXO(accepted_tx.txid_int, 0, accepted_tx.vout[0].nValue))
    ...
    def test_witness_tx_relay_before_segwit_activation(self):
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(tx_no_witness.txid_int, 0, tx_no_witness.vout[0].nValue))
    ...
    def test_standardness_v0(self):
        ...
        # self.utxo[0] has scriptPubKey [OP_TRUE, OP_DROP] * 14 + [OP_CHECKSIG] (requires 1 sigop), so signing is needed
        self.sign_utxo0_input(p2sh_tx)
        p2sh_tx.vout = [CTxOut(self.utxo[0].nValue - 1000, p2sh_script_pubkey)]
        # Ensure witness is completely empty since we're spending a non-witness output
        p2sh_tx.wit = CTxWitness()

        # Mine it on test_node to create the confirmed output.
        # Send without witness since we're spending a non-witness output
    ...
    def test_segwit_versions(self):
        ...
        if len(self.utxo) < NUM_SEGWIT_VERSIONS:
            tx = CTransaction()
            tx.vin.append(CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b""))
            # Check if we need to sign - if utxo[0] is from test_non_witness_transaction, it requires signing
            # Otherwise, it might be anyone-can-spend (CScript([OP_TRUE]))
            # For now, we'll sign it - if it's anyone-can-spend, the signature will be ignored
            self.sign_utxo0_input(tx)
        ...
        for version in list(range(OP_1, OP_16 + 1)) + [OP_0]:
            ...
            tx.vin = [CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b"")]
            # No signing needed - CScript([OP_TRUE]) is anyone-can-spend
