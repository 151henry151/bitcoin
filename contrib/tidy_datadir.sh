#!/bin/bash
# Copyright (c) 2013 The HSBC Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete HSBC database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no HSBC datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old HSBC datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected HSBC 0.7 datadir."
    ;;
  3)
    echo "Detected HSBC pre-0.8 datadir."
    ;;
  4)
    echo "Detected HSBC 0.8 datadir."
    ;;
esac
